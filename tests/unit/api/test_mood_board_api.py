...
async def test_mood_board_health(client: AsyncClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["image_configured"] is False
...
async def test_mood_board_health_local_providers(
    ...
    assert data["image_provider"] == "openai_compatible"
...
@pytest.mark.asyncio
async def test_generate_503_when_worker_unreachable(
    client: AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("CELERY_BROKER_URL", "redis://localhost:6379/0")

    with patch("copy_that.interfaces.api.mood_board.celery_app") as celery_app:
        insp = MagicMock()
