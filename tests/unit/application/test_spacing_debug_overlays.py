...
def test_spacing_debug_payload_includes_toggles_and_confidence(monkeypatch):
    ...
    monkeypatch.setattr(
        spacing_cv_extractor, "preprocess_image", lambda data: {"cv_gray": np.zeros((8, 8), dtype=np.uint8)}
    )
    monkeypatch.setattr(
        spacing_cv_extractor, "components_to_bboxes", lambda gray: [(0, 0, 2, 2), (4, 0, 2, 2)]
    )
    monkeypatch.setattr(spacing_cv_extractor, "gaps_from_bboxes", lambda bboxes, **kwargs: ([4, 8], []))
    monkeypatch.setattr(
        spacing_cv_extractor,
        "infer_grid_from_bboxes",
        lambda bboxes, canvas_width: {"columns": 2, "gutter_px": 4, "margin_left": 0, "margin_right": 0},
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(spacing_cv_extractor.su, "cross_check_gaps", lambda gaps, base_unit, **_: {})
    monkeypatch.setattr(spacing_cv_extractor.su, "cluster_gaps", lambda gaps: {"dominant": gaps[:1]})
...
def test_spacing_synthetic_layouts_produce_confidence(monkeypatch):
    ...
    monkeypatch.setattr(
        spacing_cv_extractor, "preprocess_image", lambda data: {"cv_gray": np.zeros((120, 200), dtype=np.uint8)}
    )
    monkeypatch.setattr(spacing_cv_extractor, "run_layoutparser_text", lambda *_, **__: ([], []))
    monkeypatch.setattr(spacing_cv_extractor, "run_uied", lambda *_, **__: [])
    ...
    for name, (boxes, gaps) in layouts.items():
        ...
        monkeypatch.setattr(spacing_cv_extractor, "gaps_from_bboxes", lambda *_: (gaps, []))
        monkeypatch.setattr(
            spacing_cv_extractor.su, "infer_base_spacing_robust", lambda g=gaps: (base, 0.85, g)
        )
        monkeypatch.setattr(
            spacing_cv_extractor.su, "cross_check_gaps", lambda *_: {"dominant": base, "tolerance": 1.0}
        )
        monkeypatch.setattr(spacing_cv_extractor.su, "cluster_gaps", lambda gaps: {"dominant": gaps[:1]})
        monkeypatch.setattr(
            spacing_cv_extractor, "infer_grid_from_bboxes", lambda *_: {"columns": 2, "confidence": 0.8}
        )

        extractor = CVSpacingExtractor(max_tokens=5, overlay_emit_debug=True)
        result = extractor.extract_from_bytes(f"{name}".encode("utf-8"))
