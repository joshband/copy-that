"""API contract tests for typography multi-extractor endpoint."""
...
async def test_project(async_db):
    project = Project(name="Typography Multi", description="multi extract")
...
@pytest.mark.asyncio
async def test_typography_extract_multi_endpoint_exists(client, test_project, tiny_png_b64):
    resp = await client.post(
        "/api/v1/typography/extract/multi",
    ...
    if resp.status_code == 200:
        body = resp.json()
        assert "typography_tokens" in body or "tokens" in body
