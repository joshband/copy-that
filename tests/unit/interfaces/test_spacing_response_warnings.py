...
def _token(value_px: int = 8, confidence: float = 0.5) -> SpacingToken:
    return SpacingToken(
        value_px=value_px,
...
def test_spacing_response_includes_warnings_and_common_spacings():
    base_result = SpacingExtractionResult(
        ...
        spacing_confidence_breakdown={"overall": 0.5, "fallback": 0.0},
    )
    ...
    assert response.spacing_confidence_breakdown == {"overall": 0.5, "fallback": 0.0}
...
def test_spacing_merge_keeps_cv_fallback_confidence(monkeypatch):
    """AI high confidence must not inflate an honest CV 4pt fallback."""
    monkeypatch.setattr(spacing, "SpacingToken", spacing_models.SpacingToken)
    cv = SpacingExtractionResult(
        tokens=[_token(4, confidence=0.15), _token(8, confidence=0.15)],
        scale_system=SpacingScale.FOUR_POINT,
        base_unit=4,
        base_unit_confidence=0.15,
        grid_compliance=0.0,
        extraction_confidence=0.15,
        min_spacing=4,
        max_spacing=8,
        unique_values=[4, 8],
        warnings=["Spacing CV fallback (insufficient_components): using 4pt preset"],
    ...
    ai = SpacingExtractionResult(
        tokens=[_token(4, confidence=0.9), _token(8, confidence=0.88)],
        scale_system=SpacingScale.FOUR_POINT,
        base_unit=4,
    ...
    merged = spacing._merge_spacing(cv, ai)  # type: ignore[arg-type]
    assert merged.extraction_confidence == 0.15
    assert merged.spacing_confidence_breakdown is not None
    assert merged.spacing_confidence_breakdown.get("fallback") == 1.0
    assert any("fallback" in w.lower() for w in (merged.warnings or []))


def test_spacing_merge_prefers_measured_cv_confidence(monkeypatch):
    monkeypatch.setattr(spacing, "SpacingToken", spacing_models.SpacingToken)
    cv = SpacingExtractionResult(
        tokens=[_token(8, confidence=0.93)],
        scale_system=SpacingScale.FOUR_POINT,
        base_unit=4,
        base_unit_confidence=0.9,
        grid_compliance=1.0,
        ...
        max_spacing=8,
        unique_values=[8],
        spacing_confidence_breakdown={"overall": 0.93, "fallback": 0.0},
    )
    ai = SpacingExtractionResult(
        tokens=[_token(8, confidence=0.85)],
        scale_system=SpacingScale.FOUR_POINT,
        base_unit=4,
        base_unit_confidence=0.85,
        grid_compliance=1.0,
        extraction_confidence=0.85,
        min_spacing=8,
        max_spacing=8,
    ...
    merged = spacing._merge_spacing(cv, ai)  # type: ignore[arg-type]
    assert merged.extraction_confidence == 0.93
    assert merged.spacing_confidence_breakdown is not None
    assert merged.spacing_confidence_breakdown.get("overall") == 0.93
