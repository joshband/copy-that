...
## Current State
- Legacy modules are still imported in some tests and a few integration helpers.
...
3. **Test cleanup** – drop or rewrite legacy pipeline tests once call sites move to the new path.
4. **Remove legacy code** – delete `src/copy_that/pipeline/**` after migration; update docs to point only to the token graph architecture.
