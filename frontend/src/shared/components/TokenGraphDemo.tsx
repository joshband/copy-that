...
import { getAdapter, hasAdapter, type TokenCategory as AdapterTokenCategory } from '../adapters'
...
function asAdapterCategory(category: string): AdapterTokenCategory | null {
  const known: AdapterTokenCategory[] = [
    'color',
    'spacing',
    'shadow',
    'typography',
  ...
  return known.includes(category as AdapterTokenCategory)
    ? (category as AdapterTokenCategory)
    : null
}
    ...
    <div className="token-graph-demo">
      ...
      <div className="token-graph-demo__section">
        ...
        <div className="token-graph-demo__grid">
          {allTokens.slice(0, 20).map((token) => {
            ...
            const adapterCategory = asAdapterCategory(token.category)
            const adapter =
              adapterCategory && hasAdapter(adapterCategory)
                ? getAdapter(adapterCategory)
                : null
      ...
      {selectedToken && (
        <div className="token-graph-demo__details">
          ...
          {asAdapterCategory(selectedToken.category) &&
            hasAdapter(asAdapterCategory(selectedToken.category)!) && (
            <div className="token-graph-demo__detail-card">
              <div>{getAdapter(asAdapterCategory(selectedToken.category)!).renderSwatch(selectedToken)}</div>
              <div className="token-graph-demo__detail-meta">
                {getAdapter(asAdapterCategory(selectedToken.category)!).renderMetadata(selectedToken)}
              </div>
            </div>
          )}
