...
type TokenCategory = 'color' | 'spacing' | 'shadow' | 'typography' | 'layout' | 'opacity'
...
export function useTokenGraph(): TokenGraphAPI {
  ...
  const opacity = useTokenGraphStore((s) => s.opacity)

  const allTokens = useMemo(
    () => [...colors, ...spacing, ...shadows, ...typography, ...layout, ...opacity],
    [colors, spacing, shadows, typography, layout, opacity]
  )
  ...
  const api: TokenGraphAPI = useMemo(
    ...
    [allTokens, colors, spacing, shadows, typography, layout, opacity]
  )
