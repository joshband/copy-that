...
import { ApiClient, resolveApiBase } from '../client'
...
describe('resolveApiBase', () => {
  it('defaults to relative /api/v1', () => {
    expect(resolveApiBase(undefined)).toBe('/api/v1')
    expect(resolveApiBase('')).toBe('/api/v1')
    expect(resolveApiBase('   ')).toBe('/api/v1')
  })

  it('preserves explicit /api/v1 bases', () => {
    expect(resolveApiBase('/api/v1')).toBe('/api/v1')
    expect(resolveApiBase('http://localhost:8000/api/v1')).toBe('http://localhost:8000/api/v1')
    expect(resolveApiBase('http://localhost:8000/api/v1/')).toBe('http://localhost:8000/api/v1')
  })

  it('appends /api/v1 when docs-style origin-only URL is set', () => {
    expect(resolveApiBase('http://localhost:8000')).toBe('http://localhost:8000/api/v1')
    expect(resolveApiBase('http://127.0.0.1:8000/')).toBe('http://127.0.0.1:8000/api/v1')
  })
})
