...
const ShadowTokenList: React.FC<Props> = ({
  ...
  if (displayShadows.length === 0) {
    return (
      <div className="empty-state" data-testid="shadow-empty-state">
        <p className="empty-title">No elevation detected</p>
        <p className="empty-subtitle standin">
          This image has no measurable cast or soft shadow cues. That is an honest
          empty result, not an extraction failure.
        </p>
      </div>
    )
  }
