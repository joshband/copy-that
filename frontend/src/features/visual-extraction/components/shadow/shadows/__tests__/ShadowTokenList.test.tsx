...
describe('ShadowTokenList', () => {
  ...
  describe('Empty State', () => {
    it('should render empty state when no shadows provided', () => {
      ...
      expect(screen.getByText('No elevation detected')).toBeInTheDocument()
    })
    ...
    it('should clarify empty is not a failure', () => {
      render(<ShadowTokenList shadows={[]} />)
      expect(
        screen.getByText(/honest empty result, not an extraction failure/i),
      ).toBeInTheDocument()
    })
  })
