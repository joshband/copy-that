...
describe('image-uploader hooks', () => {
  ...
  describe('useParallelExtractions', () => {
    ...
    it('extractSpacing should surface HTTP failures', async () => {
      const { result } = renderHook(() => useParallelExtractions())
      const mockFetch = vi.fn().mockResolvedValue({
        ok: false,
        status: 500,
        statusText: 'Internal Server Error',
        json: async () => ({ detail: 'Spacing backend down' }),
      })
      global.fetch = mockFetch

      let extractionResult: any
      await act(async () => {
        extractionResult = await result.current.extractSpacing('base64data', 'image/jpeg', 123)
      })

      expect(extractionResult).toEqual({ ok: false, error: 'Spacing backend down' })
    })

    it('parseColorStream should throw on stream error events', async () => {
      const { result } = renderHook(() => useStreamingExtraction())
      const mockStream = new ReadableStream({
        start(controller) {
          controller.enqueue(
            new TextEncoder().encode('data: {"error":"OpenAI quota exceeded"}\n'),
          )
          controller.close()
        },
      })
      const mockResponse = { body: mockStream, ok: true } as Response
  ...
  describe('useProjectManagement', () => {
    ...
    it('surfaces API error detail when create project fails', async () => {
      ...
      await expect(result.current.ensureProject(null, 'Test Project')).rejects.toThrow(
        /Failed to create project: Not Found/,
      )
    })
  })
})
