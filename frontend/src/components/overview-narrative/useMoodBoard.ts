...
import type { HeuristicMoodBoardVariant, HeuristicMoodBoardTheme } from './moodBoardTypes'
...
/**
 * Client-side heuristic mood boards from palette analysis (no API).
 * Product UI uses the async Celery job path in MoodBoard.tsx instead.
 */
export function useMoodBoard(colors: ColorToken[]): HeuristicMoodBoardVariant[] {
  const { temp, sat } = usePaletteAnalysis(colors)
  const movement = useArtMovementClassification(colors)

  return useMemo((): HeuristicMoodBoardVariant[] => {
    const variants: HeuristicMoodBoardVariant[] = []
