...
export const EXTRACTION_TIMEOUT_MS = Number(process.env.PLAYWRIGHT_EXTRACTION_TIMEOUT_MS ?? 6 * 60 * 1000)
const EXTRACTION_START_TIMEOUT_MS = Number(
  process.env.PLAYWRIGHT_EXTRACTION_START_TIMEOUT_MS ?? 60 * 1000,
)
...
export async function waitForTokenDataReady(
  ...
  const readCounts = async () => {
    const text = (await snapshotCard.innerText()) ?? ''
    const findCount = (label: string) => {
      const match = text.match(new RegExp(`(\\d+)\\s+${label}`, 'i'))
      return match ? Number(match[1]) : 0
    }
    return {
      colors: findCount('colors'),
      spacing: findCount('spacing'),
      typography: findCount('typography'),
      shadows: findCount('shadows'),
    }
  }
