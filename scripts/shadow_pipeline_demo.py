...
def create_synthetic_shadow_image(size: int = 512) -> np.ndarray:
    ...
    for c in range(3):
        ...
        channel[shadow_mask] = channel[shadow_mask] * (1 - shadow_soft[shadow_mask] * shadow_intensity * color_mult)
    ...
    shadow2_dist = ((x - shadow2_cx) / (obj2_radius * 1.2))**2 + ((y - shadow2_cy) / (obj2_radius * 0.4))**2
...
def create_overlay(rgb: np.ndarray, mask: np.ndarray, color: tuple = (1, 0, 0), alpha: float = 0.5) -> np.ndarray:
...
def run_demo(image_path: str | None = None, output_dir: str = "shadow_demo_output"):
    ...
    cv2.imwrite(str(output_path / "01_input.png"), cv2.cvtColor((rgb * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    cv2.imwrite(str(output_path / "02_illumination.png"), cv2.cvtColor((illum_vis * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    cv2.imwrite(str(output_path / "03_classical_candidates.png"), cv2.cvtColor((candidates_vis * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))

    # Overlay on original
    overlay_classical = create_overlay(rgb, candidates, color=(0, 1, 1), alpha=0.4)
    cv2.imwrite(str(output_path / "03_classical_overlay.png"), cv2.cvtColor((overlay_classical * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    cv2.imwrite(str(output_path / "04_ml_shadow.png"), cv2.cvtColor((ml_vis * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))

    # Overlay on original
    overlay_ml = create_overlay(rgb, ml_shadow, color=(1, 0, 1), alpha=0.4)
    cv2.imwrite(str(output_path / "04_ml_overlay.png"), cv2.cvtColor((overlay_ml * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    cv2.imwrite(str(output_path / "06_depth.png"), cv2.cvtColor((depth_vis * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))

    # Normals
    print("[Stage 06] Computing surface normals...")
    normals, normals_vis = depth_to_normals(depth)
    cv2.imwrite(str(output_path / "06_normals.png"), cv2.cvtColor((normals_vis * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    ...
    cv2.imwrite(str(output_path / "00_comparison_grid.png"), cv2.cvtColor((grid * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
