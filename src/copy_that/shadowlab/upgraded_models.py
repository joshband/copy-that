...
class BDRARShadowDetector:
    ...
    def detect(
        ...
        try:
            ...
            if logits.ndim == 4:
                ...
                shadow_mask = torch.argmax(logits, dim=1).squeeze().cpu().numpy() > 0
    ...
    def _fallback_shadow_detection(self, rgb_image: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    def _load_model(self):
        ...
        try:
            ...
            try:
                ...
                self.model = torch.hub.load("intel-isl/MiDaS", "DPT_Large", pretrained=True).to(
                    self.device
                )
                self.model.eval()
            ...
            logger.warning(
                "PyTorch not installed. ZoeDepth unavailable. Install torch for depth estimation."
            )
...
class IntrinsicNetDecomposer:
    ...
    def decompose(self, rgb_image: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def _fallback_intrinsic_decomposition(
        ...
        reflectance_bgr = (
            cv2.bilateralFilter(rgb_uint8, d=15, sigmaColor=75, sigmaSpace=75).astype(np.float32)
            / 255.0
        )

        # Shading = normalized ratio
        shading = np.clip(
            np.mean(rgb_image, axis=2) / (np.mean(reflectance_bgr, axis=2) + 1e-8),
            0,
            1,
        ).astype(np.float32)
...
class OmnidataNormalEstimator:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.warning(
                "PyTorch not installed. Omnidata unavailable. Install torch for normal estimation."
            )
