...
def _device_to_geometry_profile(device: str) -> GeometryProfile:
    """Map lighting device hint to a P4 geometry profile (no stand-in path)."""
    normalized = (device or "cpu").strip().lower()
    if normalized in {"cuda", "gpu", "gpu_full", "mps"}:
        return GeometryProfile.GPU_FULL
    if normalized in {"cpu_accurate", "accurate"}:
        return GeometryProfile.CPU_ACCURATE
    if normalized in {"cpu_fast", "cpu"}:
        return GeometryProfile.CPU_FAST
    return GeometryProfile.AUTO


def _extract_real_geometry(
    image_bgr: np.ndarray,
    *,
    device: str,
    geometry_profile: GeometryProfile | None,
) -> tuple[np.ndarray | None, np.ndarray | None, dict[str, Any] | None]:
    """Run P4 geometry extract; never fall back to shadowlab stand-in depth."""
    profile = geometry_profile or _device_to_geometry_profile(device)
    image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
    pil = Image.fromarray(image_rgb)
    geo = extract_depth_and_normals(pil, profile=profile)
    return geo["depth01"], geo["normals_xyz"], dict(geo.get("meta") or {})
...
def compute_classical_extraction_confidence(features: ShadowFeatures) -> float:
    """
    CPU-cheap classical confidence from area / softness / contrast cues.

    Does not rely on light_direction_confidence (often 0 when use_geometry=False).
    Flat / no-cue images stay near 0; images with real cast/soft shadows land ~0.35–0.8.
    """
    area = float(np.clip(features.shadow_area_fraction, 0.0, 1.0))
    if area < 0.002:
        return 0.0

    # Sparse but visible peaking ~15% coverage; heavy coverage saturates.
    area_score = float(np.clip(area / 0.15, 0.0, 1.0))
    softness_score = float(np.clip(features.edge_softness_mean, 0.0, 1.0))
    contrast_score = float(np.clip(features.shadow_contrast / 0.5, 0.0, 1.0))
    intensity_gap = float(
        np.clip(abs(features.mean_lit_intensity - features.mean_shadow_intensity), 0.0, 1.0)
    )
    ...
    light = float(np.clip(features.light_direction_confidence, 0.0, 1.0))
    if light > 0.05:
        classical = 0.7 * classical + 0.3 * light

    return float(np.clip(classical, 0.0, 0.85))
...
def quantize_shadow_tokens(features: ShadowFeatures) -> ShadowTokens:
    ...
    return ShadowTokens(
        ...
        extraction_confidence=compute_classical_extraction_confidence(features),
...
def analyze_image_for_shadows(
    ...
    Orchestrates classical detection, P4 geometry depth/normals (when enabled),
    ...
    Args:
        ...
        use_geometry: Whether to run real geometry extract for depth/normals
        device: Compute hint mapped to GeometryProfile ("cpu", "cuda", "mps", …)
        geometry_profile: Optional explicit GeometryProfile (overrides device mapping)

    Returns:
        Dictionary containing:
            - shadow_soft: Soft shadow map (H×W float32, 0..1)
            - shadow_mask: Binary mask (H×W uint8, 0-255)
            - depth: Depth map if geometry succeeded (H×W float32)
            - normals: Surface normals if geometry succeeded (H×W×3 float32)
            - geometry_meta: Geometry extract meta when use_geometry=True
    ...
    """
    _ = use_deep  # reserved for future deep classical paths
    ...
    # Step 2: Real geometry extract (optional) — no shadowlab stand-in fallback
    depth = None
    normals = None
    geometry_meta: dict[str, Any] | None = None
    if use_geometry:
        try:
            depth, normals, geometry_meta = _extract_real_geometry(
                image_bgr,
                device=device,
            ...
            geometry_meta = {"error": "extract_failed", "detail": str(exc)}
    ...
    try:
        ...
        logger.warning("Intrinsic decomposition failed: %s", exc)
        shading = None
