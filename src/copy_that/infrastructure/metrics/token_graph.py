...
class TokenGraph:
    ...
    def _create_token_node(self, token: Base, category: str) -> TokenNode:
        ...
        token_id_raw = getattr(token, "id", None)
        if token_id_raw is None:
            raise ValueError(f"Token of category '{category}' is missing an id")
    ...
    def _extract_primary_value(self, token: Base, category: str) -> Any:
        ...
        """
        value_map: dict[str, Callable[[Base], Any]] = {
        ...
        extractor: Callable[[Base], Any] = value_map.get(category, lambda _: None)
