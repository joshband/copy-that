...
def build_typography_repo(
    ...
    for index, token in enumerate(tokens, start=1):
        ...
        # Phase 2: dual-write atomic fontFamily / fontWeight with COMPOSES
        family_atom = make_font_family_token(str(font_family), source="extracted")
        try:
            weight_int = int(font_weight)
        except (TypeError, ValueError):
            weight_int = 400
        weight_atom = make_font_weight_token(weight_int, source="extracted")
        if repo.get_token(family_atom.id) is None:
            repo.upsert_token(family_atom)
        if repo.get_token(weight_atom.id) is None:
            repo.upsert_token(weight_atom)

        repo.upsert_token(
            make_typography_token(
                f"{namespace}/{semantic_role}/{index:02d}",
                font_family_token_id=family_atom.id,
                font_weight_token_id=weight_atom.id,
...
def aggregate_typography_batch(
    ...
    # Build repository from merged tokens (dual-write font atoms)
    repo = InMemoryTokenRepository()
