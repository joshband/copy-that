...
def build_spacing_repo(
    ...
    Phase 2: dual-write DTCG ``dimension`` companions (Compat+ keeps ``spacing``).
    """
    ...
    for index, token in enumerate(tokens, start=1):
        ...
        spacing = make_spacing_token(
            f"{namespace}/{index:02d}",
            value_px,
            value_rem,
            attrs,
        )
        companion = make_dimension_companion(spacing, source="extracted")
        if companion is not None:
            spacing = attach_dimension_companion(spacing, companion)
            if repo.get_token(companion.id) is None:
                repo.upsert_token(companion)
...
def merge_spacing(
    ...
    Prefer measured CV confidence; never let AI inflate a CV fallback score.
    """
    ...
    cv_fallback = (
        isinstance(breakdown, dict) and float(breakdown.get("fallback") or 0) >= 1
    ) or any("fallback" in str(w).lower() for w in (getattr(cv, "warnings", None) or []))
    cv_conf = float(getattr(cv, "extraction_confidence", 0) or 0)
    ai_conf = float(getattr(ai, "extraction_confidence", 0) or 0)
    if cv_fallback:
        extraction_confidence = cv_conf if cv_conf > 0 else 0.15
        confidence_breakdown = getattr(cv, "spacing_confidence_breakdown", None)
    elif cv_conf > 0:
        extraction_confidence = cv_conf
        confidence_breakdown = getattr(cv, "spacing_confidence_breakdown", None) or getattr(
            ai, "spacing_confidence_breakdown", None
        )
    else:
        extraction_confidence = ai_conf
        confidence_breakdown = getattr(ai, "spacing_confidence_breakdown", None) or getattr(
            cv, "spacing_confidence_breakdown", None
        )
    ...
    return SpacingExtractionResult(
        ...
        base_unit_confidence=(
            cv.base_unit_confidence if cv_fallback or cv_conf > 0 else ai.base_unit_confidence
        )
        or ai.base_unit_confidence
        or cv.base_unit_confidence,
