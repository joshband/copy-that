...
_TYPE_OVERRIDES: dict[TokenType, str] = {
    ...
    TokenType.FONT_FAMILY_DTCG: "fontFamily",
    TokenType.FONT_SIZE: "dimension",
    TokenType.OPACITY: "number",
}
...
def tokens_to_w3c(repo: TokenRepository) -> dict[str, Any]:
    ...
    for token in tokens:
        ...
        if _is_alias(token):
            ...
            # String-typed synth tokens (fontFamily, border, transition, …)
            dtcg_type = token.attributes.get("$type") or _type_name(token.type)
            entry = {"$type": dtcg_type, "$value": token.value}
...
def w3c_to_tokens(data: dict[str, Any], repo: TokenRepository) -> None:
    ...
    section_map: dict[str, TokenType | str] = {
...
def _token_to_w3c_typography_entry(token: Token, hex_to_id: dict[str, str]) -> dict[str, Any]:
    ...
    if isinstance(font_weight, str):
        entry["$value"]["fontWeight"] = (
            _wrap_ref(font_weight) if _looks_like_token_ref(font_weight) else font_weight
        )
    elif font_weight is not None:
        entry["$value"]["fontWeight"] = font_weight
...
def _w3c_typography_entry_to_token(
    ...
    font_weight = raw_value.get("fontWeight")
    if isinstance(font_weight, str) and font_weight.startswith("{") and font_weight.endswith("}"):
        weight_id = font_weight.strip("{}")
        value["fontWeight"] = weight_id
        rels.append(
            TokenRelation(
                type=RelationType.COMPOSES, target=weight_id, meta={"role": "font-weight"}
            )
        )
    elif font_weight is not None:
...
# Map internal / synth type strings onto stable W3C section roots (Compat+).
_SECTION_ALIASES: dict[str, str] = {
    "font.family": "fontFamily",
    "font.size": "dimension",
    "opacity": "opacity",  # values use $type number
    "layout.grid": "layout",
}


def _section_for_type(token_type: TokenType | str) -> str | None:
    raw = token_type.value if isinstance(token_type, TokenType) else str(token_type)
