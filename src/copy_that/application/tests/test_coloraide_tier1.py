...
def _distance_pair(result: str | tuple[str, float]) -> tuple[str, float]:
...
class TestPaletteMatching:
    ...
    def test_match_distance_is_zero_for_identical(self):
        ...
        palette = ["#FF0000"]
        _, distance = _distance_pair(
            match_color_to_palette("#FF0000", palette, return_distance=True)
        )
    ...
    def test_match_distance_increases_with_difference(self):
        ...
        _, d_identical = _distance_pair(
            match_color_to_palette("#FF0000", palette, return_distance=True)
        )
        _, d_slightly_different = _distance_pair(
            match_color_to_palette("#FF1111", palette, return_distance=True)
        )
        _, d_very_different = _distance_pair(
            match_color_to_palette("#0000FF", palette, return_distance=True)
        )
...
class TestIntegrationGamutAndMatching:
    ...
    def test_design_system_standardization(self):
        ...
        for extracted in extracted_colors:
            ...
            standardized, distance = _distance_pair(
                match_color_to_palette(displayable, design_system, return_distance=True)
            )
