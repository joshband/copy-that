...
class ColorKMeansClustering:
    ...
    def extract_palette(self, image: np.ndarray) -> list[ColorClusterResult]:
        ...
        for cluster_id in range(self.k):
            ...
            # Compactness: tighter LAB/RGB clusters → higher confidence
            if len(cluster_pixels) > 1:
                dists = np.linalg.norm(
                    cluster_pixels.astype(np.float64) - centers[cluster_id].astype(np.float64),
                    axis=1,
                )
                compactness = float(1.0 / (1.0 + float(dists.std()) / 28.0))
            else:
                compactness = 0.45
            share = pixel_count / max(total_pixels, 1)
            cluster_conf = min(
                0.95,
                max(0.3, 0.38 + 0.42 * min(1.0, share * 6.0) + 0.15 * compactness),
            )
