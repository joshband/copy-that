...
class ShadowAggregator:
    ...
    def aggregate_tokens(self, tokens_by_extractor: list[list[ShadowStyle]]) -> list[ShadowStyle]:
        ...
        # Prefer higher-confidence shadows, then nearer-to-origin configurations
        all_tokens.sort(
            key=lambda t: (-float(t.confidence), math.sqrt(t.x**2 + t.y**2 + t.blur**2))
        )

        # Deduplicate using similarity check
        deduplicated: list[ShadowStyle] = []
        for token in all_tokens:
            is_duplicate = False
            for existing in deduplicated:
                if self._are_similar(token, existing):
                    if existing.extraction_metadata is None:
                    ...
                    existing.extraction_metadata["extractor_sources"].append(
                        (token.extraction_metadata or {}).get("extractor_index")
                    )
