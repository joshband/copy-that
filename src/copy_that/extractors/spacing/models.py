...
class SpacingExtractionResult(BaseModel):
    ...
    cv_distance_candidates: list[dict[str, Any]] | None = Field(
        default=None,
        description="Raw CV adjacency / distance candidates used for confidence.",
    )
    spacing_confidence_breakdown: dict[str, float] | None = Field(
        default=None,
        description="measurement/grid/semantic/overall scores; fallback=1.0 when preset.",
    )
    alignment_groups: list[dict[str, Any]] | None = Field(
        default=None,
        description="Grouped alignment lines for layout QA.",
    )
    elevation_tokens: list[dict[str, Any]] | None = Field(
        default=None,
        description="Optional elevation-derived spacing tokens.",
    )
