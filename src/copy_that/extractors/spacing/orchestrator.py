...
Aggregator note: this module keeps a local ``SpacingAggregator`` (4px distance)
aligned with the orchestrator token contract (``extractors.spacing.models``).
``tokens.spacing.aggregator.SpacingAggregator`` is used by the batch spacing API
and operates on application spacing models — do not mix them without mapping.
"""
