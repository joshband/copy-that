...
from copy_that.extractors.cv_helpers.layout_text_detector import (
    ImageMode,
    TextToken,
...
class CVSpacingExtractor:
    ...
    def __init__(
        ...
        # Default OFF for happy-path latency (opt-in via FASTSAM_ENABLED=1).
        self._fastsam_enabled = (
            fastsam_enabled
            if fastsam_enabled is not None
            else (enabled_env is not None and enabled_env.lower() in {"1", "true", "yes"})
        )
        ...
        # Default OFF — UIED subprocess is slow; opt-in via ENABLE_UIED=1.
        uied_env = os.getenv("ENABLE_UIED", "0")
        self._uied_enabled = uied_env.lower() in {"1", "true", "yes"}
        # Default OFF — MiDaS/Laplacian depth is optional refinement, not spine.
        depth_env = os.getenv("ENABLE_SPACING_DEPTH", "0")
        self._depth_enabled = depth_env.lower() in {"1", "true", "yes"}
    ...
    def extract_from_bytes(self, data: bytes) -> SpacingExtractionResult:
        if cv2 is None:
            return self._fallback(reason="opencv_missing")
        try:
            views = preprocess_image(data)
            gray = views["cv_gray"]
            bgr = views["cv_bgr"]
            try:
                bgr, gray, text_boxes = apply_text_mask(bgr, gray)
                views["cv_bgr"] = bgr
                views["cv_gray"] = gray
                views["text_boxes"] = text_boxes
            except Exception as exc:  # noqa: BLE001
                logger.debug("Text mask skipped: %s", exc)
                views["text_boxes"] = []
        except Exception:
            return self._fallback(reason="preprocess_failed")
        ...
        depth_map = None
        if self._depth_enabled and estimate_depth_map is not None:
            depth_map = estimate_depth_map(gray)
        ...
        if len(bboxes) < 2:
            return self._fallback(reason="insufficient_components")
        ...
        if not all_gaps:
            return self._fallback(reason="no_gaps")

        # Prefer repeated component/edge gaps over one-off pixel noise when inferring base.
        gap_int_counts = Counter(int(round(g)) for g in all_gaps if g and g > 0)
        top_modes = {g for g, _ in gap_int_counts.most_common(5)}
        clustered_gaps = [
            g
            for g in all_gaps
            if gap_int_counts.get(int(round(g)), 0) >= 2 or int(round(g)) in top_modes
        ]
        gaps_for_base = clustered_gaps if len(clustered_gaps) >= 4 else all_gaps

        base_unit, base_confidence, normalized_values = su.infer_base_spacing_robust(gaps_for_base)
        if not normalized_values:
            normalized_values = su.cluster_spacing_values(all_gaps, tolerance=0.15)
        if not normalized_values:
            return self._fallback(reason="no_scale")
        ...
        for i, (label, v) in enumerate(zip(self._labels(), values, strict=False)):
            ...
            cluster_boost = 0.12 if counts.get(v, 0) >= 3 and grid_aligned else 0.0
            tokens.append(
                SpacingToken(
                    value_px=int(v),
                    ...
                    spacing_type=None,
                    category="cv",
                    confidence=min(0.92, 0.55 + (0.1 * min(prominence, 0.3)) + cluster_boost),
                    usage=["layout"],
                    scale_position=i,
                    base_unit=base_unit,
        ...
        layout_image = pil_img
        if layout_image is None:
            layout_image = views.get("cv_bgr")
        if layout_image is None:
            layout_image = gray
        ...
        # Sub-4 measured bases are floored to 4 with low base confidence —
        # do not advertise a high overall score for a noise-derived scale.
        if base_confidence <= 0.25:
            extraction_confidence = min(float(extraction_confidence), 0.35)
    ...
    @staticmethod
    def _fallback(reason: str = "unknown") -> SpacingExtractionResult:
        """Deterministic 4pt preset when CV cannot measure gaps.

        Confidence is intentionally low so callers do not treat this as
        screenshot evidence.
        """
        ...
        return SpacingExtractionResult(
            ...
            scale_system=SpacingScale.FOUR_POINT,
            base_unit=4,
            base_unit_confidence=low_conf,
            grid_compliance=0.0,
            extraction_confidence=low_conf,
