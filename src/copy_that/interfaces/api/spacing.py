...
async def _image_bytes_from_spacing_request(
    ...
    if request.image_url:
        data, content_type = await async_executor.run(
            lambda: _download_image_bytes(str(request.image_url))
        )
...
async def extract_spacing_multi(
    ...
    try:
        ...
        image_bytes, _media = await _image_bytes_from_spacing_request(request, async_executor)
        extractors = [
...
async def extract_spacing_streaming(
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # Prefer CV+AI merge (same spine as /extract) so shape tokens persist
            yield _format_sse_event(
                "progress",
                {
                    "status": "analyzing",
            ...
            yield _format_sse_event(
                "complete",
                {**response.model_dump(), "job_id": job_id, "project_id": project_id},
            )
